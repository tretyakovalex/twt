...
const compile = async function(data) {
    const filePath = path.join(__dirname, 'views', `purchaseCertificateTemplate.handlebars`);
...
async function generatePdf(templateData, templateName) {
    try {
        ...
        const htmlFilePath = path.join(__dirname, `compiled-purchaseCertificate.html`);
        await fs.writeFile(htmlFilePath, content);

        await page.goto(`file:${htmlFilePath}`, { waitUntil: 'networkidle0' });

        const pdfPath = path.join(__dirname, 'purchasingCertificates', `${templateName}.pdf`);
        await page.pdf({
            path: pdfPath,
            format: 'A4',
