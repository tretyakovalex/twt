...
router.get('/getResultBySampleNumber', async (req, res) => {
    const sample_number = req.query.sample_number;
    console.log("Printing sample_number (9): ", sample_number);

    try {
        twt.query('select * from results where sample_number = ?', sample_number, (err, results) => {
            if(err){
                console.error(err);
...
router.get('/getResultsByDate', async (req, res) => {
    const date = req.query.date;
    console.log("trying to print date (9): ", date);
    try {
...
router.post('/addResults', async (req, res) => {
    try {
        ...
        console.log("Printing data (89): ", data);
